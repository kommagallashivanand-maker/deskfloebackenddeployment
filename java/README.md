...
DeskFlow is the core Java Spring Boot service for the internal helpdesk platform. It provides APIs for managing customer support tickets, enforcing workflow state machine transitions, logging audit activities, and handling S3 file attachments.
...
Below is the database schema representing the 8 core tables: `teams`, `categories`, `users`, `tickets`, `ticket_attachments`, `ticket_activities`, `ticket_comments`, and `sla_policies`.
...
When the application is running locally, you can access Swagger documentation:
